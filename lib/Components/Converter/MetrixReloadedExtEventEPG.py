...
class MetrixReloadedExtEventEPG(Converter, object):
	...
	LEADTEXT = "Leadtext"										# (nur EpgShare) optional mit Prefix angabe -> z.B. Leadtext(Kurzbeschreibung:)
	CONCLUSION = "Conclusion"									# (nur EpgShare) optional mit Prefix angabe -> z.B. Conclusion(Kritik:)
				...
				for type in self.types:
					...
					if type == self.IS_EPGSHARE_AVAILABLE:
						...
						if(self.EXTENDED_DESCRIPTION_CLEAN in type):
							...
							extendedDescription = self.getExtendedDescription(type, values, event, False)
	...
	def getExtendedDescription(self, type, values, event, clean):
		desc = None
